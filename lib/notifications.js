...
exports.subscribe = function(req, res, cmds) {
  ...
  let hash = subscription.accountId + subscription.channel_id;
  ...
  // load account info to find out if it exist
...
exports.list = function(req, res) {
  ...
  for (let s in subscriptions) {
    const channel = '<#' + subscriptions[s].channel_id + '|' +
                    subscriptions[s].channel_name + '>';
...
function createStream(teamId, hash) {
  ...
  function newPayment(payment) {
    ...
    let text = amount + ' ' + asset + ' from `' + payment.from +
               '` to `' + payment.to + '`.';
    text += process.env.BLOCKCHAIN_EXPLORER ?
      '\nThe transaction: ' + process.env.BLOCKCHAIN_EXPLORER +
    ...
    if (payment.to !== accountId) {
      ...
      text = character.getRandomPrefix() + 'You just received ' + text;
  ...
  function successfulSlackPost(payment) {
    logger.info('slack post was successful, saving paging token ' +
      `${payment.paging_token}`);
    authorizations[teamId].subscriptions[hash].lastSavedToken =
      payment.paging_token;
    flushStore();
  ...
  function streamErr(err) {
    ...
    const channel = '<#' + subscription.channel_id + '|' +
    ...
    if (err.status === 404) {
      const text = 'The subscription of `' + accountId + '` for the channel ' +
                   ...
                   JSON.stringify(err) + '```If you think this could be a ' +
                   'bug please let the developer know: ' +
                   'https://github.com/cmichi/' +
                   'stellar-slack-payment-notifications#support.';
  ...
  authorizations[teamId].subscriptions[hash].closeStream = payments.stream({
    ...
    onerror: function(error) {
      logger.error('Error in payment stream: ' +
                   ...
                   'lastToken: ' + JSON.stringify(lastToken) + ', ' +
                   'accountId: ' + accountId);
